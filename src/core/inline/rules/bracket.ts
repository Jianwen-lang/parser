...
interface ParsedShapeAttribute {
  shape: 'square' | 'rounded';
  roundedRadius?: number;
}

function parseShapeAttributeFromToken(token: string): ParsedShapeAttribute | undefined {
  const value = token.trim();
  if (value === 'square') {
    return { shape: 'square' };
  }
...
function hasInlineAttributes(attrs: InlineAttributes): boolean {
  return Boolean(
    attrs.color ||
