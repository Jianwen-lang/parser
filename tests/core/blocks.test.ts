...
describe('JianWen blocks - hr, tags and comments', () => {
  ...
  it('keeps unknown attribute-only lines and reports warnings instead of dropping content', () => {
    const source = ['[unknown]', 'next line'].join('\n');
    const { ast, errors } = parseJianwenWithErrors(source);

    expect(
      errors.some(
        (e) =>
          e.code === 'unknown-attribute-token' ||
          e.message.includes('Unknown block attribute token'),
      ),
    ).toBe(true);
