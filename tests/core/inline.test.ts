...
describe('JianWen inline - links and comments', () => {
  it('parses line-start [link]text(url) into inline link nodes', () => {
    const source = '[link]Click here(https://example.com)';
    const { ast, errors } = parseJianwenWithErrors(source);
    expect(errors).toHaveLength(0);
    ...
    const link = paragraph.children.find((node): node is LinkNode => node.type === 'link');
...
describe('JianWen inline - line-start attrs and literal separators', () => {
  it('parses line-start [attrs]text[/] as inline attributes instead of raw block', () => {
    const source = '[bold]line-start attrs[/]';
    const { ast, errors } = parseJianwenWithErrors(source);
    expect(errors).toHaveLength(0);
    ...
    const attrsNode = paragraph.children.find(
      (node): node is InlineAttrsNode => node.type === 'inlineAttrs',
    );
    expect(attrsNode).toBeDefined();
    expect(attrsNode?.attrs.fontStyle).toEqual(['bold']);
  });

  it('does not treat word separators as style markers in plain text', () => {
    const source = 'well-known path/to/file 2024/03/01 email_a@example.com a^b';
    const { ast, errors } = parseJianwenWithErrors(source);
    expect(errors).toHaveLength(0);
  ...
  it('applies [rounded=num] to frame highlight shape', () => {
    const source = 'x [rounded=2]``frame`` y';
    const { ast, errors } = parseJianwenWithErrors(source);
    expect(errors).toHaveLength(0);
    ...
    const highlightFromInlineAttrs = paragraph.children
      .filter((node): node is InlineAttrsNode => node.type === 'inlineAttrs')
      .flatMap((node) => node.children)
      .find((node): node is HighlightNode => node.type === 'highlight');
    const directHighlight = paragraph.children.find(
      (node): node is HighlightNode => node.type === 'highlight',
    );

    const highlight = highlightFromInlineAttrs ?? directHighlight;
    if (!highlight) {
      throw new Error('Expected highlight node');
    }
    expect(highlight.shape).toBe('rounded');
    expect(highlight.roundedRadius).toBe(2);
  });
});
